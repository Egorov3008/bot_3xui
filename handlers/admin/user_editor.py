...
async def prompt_tg_id(callback_query: CallbackQuery, state: FSMContext):
    """
    Запрашивает у пользователя ввод tg_id клиента.

    :param callback_query: Объект callback_query от Telegram, содержащий информацию о событии.
    :param state: Контекст состояния для хранения данных о состоянии пользователя.
    """
...
async def handle_tg_id_input(message: types.Message, state: FSMContext):
    """
    Обрабатывает ввод tg_id пользователя и отображает информацию о пользователе.

    :param message: Объект сообщения от Telegram, содержащий текст от пользователя.
    :param state: Контекст состояния для хранения данных о состоянии пользователя.
    """
...
async def process_balance_change(callback_query: CallbackQuery, state: FSMContext):
    """
    Запрашивает новую сумму баланса для указанного tg_id.
...
async def handle_new_balance_input(message: types.Message, state: FSMContext):
    """
    Обрабатывает ввод новой суммы баланса и обновляет баланс пользователя в базе данных.
...
async def process_key_edit(callback_query: CallbackQuery):
    """
    Обрабатывает запрос на редактирование ключа по его email.
    ...
    try:
        ...
        try:
            ...
            if record:
                ...
                change_expiry_button = types.InlineKeyboardButton(text='⏳ Изменить время истечения',
                                                                  callback_data=f'change_expiry|{email}')
                delete_button = types.InlineKeyboardButton(text='❌ Удалить ключ',
...
async def prompt_key_name(callback_query: CallbackQuery, state: FSMContext):
    """
    Обрабатывает нажатие на кнопку поиска по имени ключа.

    Отправляет пользователю сообщение с просьбой ввести имя ключа
    и устанавливает состояние ожидания имени ключа.

    Args:
        callback_query (CallbackQuery): Объект обратного вызова, содержащий информацию о нажатой кнопке.
        state (FSMContext): Объект контекста состояния для хранения состояния пользователя.
    """
...
async def handle_key_name_input(message: types.Message, state: FSMContext):
    """
    Обрабатывает ввод имени ключа от пользователя.

    Выполняет поиск записей пользователя по введенному имени ключа.
    Если записи найдены, формирует и отправляет пользователю информацию о ключах.
    Если записи не найдены, уведомляет пользователя об отсутствии результатов.

    Args:
        message (types.Message): Сообщение, содержащее введенное пользователем имя ключа.
        state (FSMContext): Объект контекста состояния для управления состоянием пользователя.
    """
    ...
    try:
        ...
        for record in user_records:
            ...
            change_expiry_button = InlineKeyboardButton(text='⏳ Изменить время истечения',
                                                        callback_data=f'change_expiry|{email}')
...
async def prompt_expiry_change(callback_query: CallbackQuery, state: FSMContext):
    """
    Запрашивает у пользователя новое время истечения для ключа.

    Отправляет пользователю сообщение с просьбой ввести новое время истечения
    для указанного ключа и обновляет состояние для ожидания ввода времени.
...
async def handle_expiry_time_input(message: types.Message, state: FSMContext):
    """
    Обрабатывает ввод времени истечения ключа от пользователя.

    Функция ожидает, что пользователь введет время истечения в формате 'YYYY-MM-DD HH:MM:SS'.
    Если введенное время корректно, обновляет время истечения ключа клиента в базе данных
    и синхронизирует изменения с панелью администратора. Если возникают ошибки,
    отправляет соответствующие сообщения пользователю.

    Args:
        message (types.Message): Сообщение от пользователя, содержащее время истечения.
        state (FSMContext): Контекст состояния, содержащий данные пользователя.
    """
    user_data = await state.get_data()
...
async def process_callback_delete_key(callback_query: types.CallbackQuery):
    """
    Обрабатывает запрос на удаление ключа от пользователя.

    Функция извлекает email из данных обратного вызова и ищет соответствующий client_id в базе данных.
    Если клиент найден, запрашивает подтверждение на удаление ключа. В противном случае
    отправляет сообщение об ошибке.

    Args:
        callback_query (types.CallbackQuery): Обратный вызов от пользователя с запросом на удаление ключа.
    """
    ...
    try:
        ...
        await bot.edit_message_text("<b>Вы уверены, что хотите удалить ключ?</b>", chat_id=tg_id,
                                    message_id=callback_query.message.message_id, reply_markup=confirmation_keyboard,
                                    parse_mode="HTML")
    finally:
        await conn.close()
...
async def process_callback_confirm_delete(callback_query: types.CallbackQuery):
    """
    Подтверждает удаление ключа клиента.

    Обрабатывает подтверждение удаления ключа для клиента.
    Извлекает идентификатор клиента и выполняет операцию удаления как в базе данных, так и через API.

    Args:
        callback_query (types.CallbackQuery): Данные обратного вызова от пользователя.
    """
    tg_id = callback_query.from_user.id
    ...
    try:
        ...
        try:
            ...
            await bot.edit_message_text(response_message, chat_id=tg_id, message_id=callback_query.message.message_id,
        ...
        await bot.edit_message_text(f"Ошибка при удалении ключа: {e}", chat_id=tg_id,
...
async def back_to_user_editor(callback_query: CallbackQuery):
    """
    Возвращает пользователя в меню редактора пользователей.

    Обрабатывает запрос на возврат к меню редактора и вызывает соответствующую функцию.

    Args:
        callback_query (CallbackQuery): Данные обратного вызова от пользователя.
    """
...
async def handle_error(tg_id, callback_query, message):
    """
    Обрабатывает ошибку и отправляет сообщение об этом пользователю.

    Отправляет пользователю сообщение с информацией об ошибке, используя идентификатор чата и идентификатор сообщения.
