...
from database import add_balance_to_client, check_connection_exists, update_key_expiry, get_client_id_by_email, \
...
async def cmd_add_balance(message: types.Message):
    """
    Обрабатывает команду /add_balance для добавления баланса клиенту.

    Эта функция проверяет, является ли пользователь администратором, и если да,
    то пытается увеличить баланс указанного клиента на указанную сумму.

    Параметры:
    - message (types.Message): Объект сообщения, содержащий команду и параметры.

    Исключения:
    - ValueError: Выбрасывается, если параметры команды не могут быть преобразованы в нужный формат (например, если не указаны client_id или amount).
    - Exception: Обрабатывает любые другие ошибки, которые могут возникнуть во время выполнения.

    Ответы:
    - Отправляет сообщение о результате операции пользователю (успех или ошибка).
    """
...
async def cmd_update_key_expiry(message: types.Message):
    """
    Обрабатывает команду /update_key_expiry для обновления времени истечения ключа клиента.

    Эта функция проверяет, является ли пользователь администратором, и если да,
    то обновляет время истечения ключа для указанного клиента и синхронизирует это изменение с панелью управления.

    Параметры:
    - message (types.Message): Объект сообщения, содержащий команду и параметры.

    Исключения:
    - ValueError: Выбрасывается, если параметры команды не могут быть преобразованы в нужный формат (например, если не указаны email или expiry_time).
    - Exception: Обрабатывает любые другие ошибки, которые могут возникнуть во время выполнения.
